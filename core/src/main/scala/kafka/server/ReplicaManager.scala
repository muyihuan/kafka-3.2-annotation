...
class ReplicaManager(val config: KafkaConfig,
  ...
  // 一旦发现有下线或者不可用的文件立刻终止进程，例如不小心删除一个分区日志文件，系统察觉到会立刻终止进程
  ...
  // Visible for testing 选择副本 可以自定义实现
  ...
  def startup(): Unit = {
    ...
    // 处理Isr相关 follower副本是否正常同步Leader副本
  ...
  def appendRecords(timeout: Long,
    ...
    if (isValidRequiredAcks(requiredAcks)) {
      ...
      // requiredAcks 不等于 -1都leader副本保存好了就直接返回
  ...
  private def appendToLocalLog(internalTopicsAllowed: Boolean,
    ...
    entriesPerPartition.map { case (topicPartition, records) =>
      ...
      // 该请求来源是否可以生产内部topic的事件消息
      if (Topic.isInternal(topicPartition.topic) && !internalTopicsAllowed) {
        // 如果不允许 不进行处理 设置响应信息为失败 无效topic 不允许添加内部topic事件消息
   ...
   * 拉取消息
   * timeout 超时时间
   * replicaId 不为空follower副本的请求
   * fetchMinBytes
   * fetchMaxBytes
   * fetchInfos topic、分区、偏移量等
   * isolationLevel 事物隔离级别
   * clientMetadata 一般follower副本的请求才有数据
   */
    ...
    readResults.map { case (topicIdPartition, readResult) =>
      ...
      val updatedReadResult = if (readResult.error != Errors.NONE) {
        ...
        onlinePartition(topicIdPartition.topicPartition) match {
          case Some(partition) =>
            // 更新follower副本的状态
            if (partition.updateFollowerFetchState(
              // follower副本的brokerId
              followerId,
              //
  ...
  // 通过查询分区副本的高水位，持久化到同级文件下的 replication-offset-checkpoint 文件
