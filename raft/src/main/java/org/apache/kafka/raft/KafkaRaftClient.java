...
public class KafkaRaftClient<T> implements RaftClient<T> {
    ...
    // 配置相关 参与投票的参与人列表，必须都是controller节点等
    private final RaftConfig raftConfig;
    private final KafkaRaftMetrics kafkaRaftMetrics;
    // 参与者的状态 ： 角色类型、选举、投票箱、角色转换等
    ...
    public void initialize() {
        // 初始化当前参与者的状态  上次endOffset & 上次选举的纪元
        ...
        // 如果只有一个参与投票的人，那就立即变为候选人进行选举，领导人肯定也必然是自己
        // 1.判断是否有投票资格 只有controller节点才有投票资格 & 并且票箱里有自己给自己的投票
        // 2.参与投票的还有没有其他人，如果没有就直接触发选举
        // 3.该节点不是候选人
        if (quorum.isVoter()
            && quorum.remoteVoters().isEmpty()
            && !quorum.isCandidate()) {

            // 转化为候选人 -> 触发选举 -> 选举出新的领导者
    ...
    private VoteResponseData handleVoteRequest(
        ...
        // 如果可以可以获取投票 且 当前节点是未参与状态 更新为投票状态
    ...
    private long pollLeader(long currentTimeMs) {
        ...
        long timeUntilSend = maybeSendRequests(
            ...
            // 领导者ID和纪元 请求类型：BEGIN_QUORUM_EPOCH
    ...
    private long pollFollowerAsObserver(FollowerState state, long currentTimeMs) {
        // leader节点超时了 随机从其他follower拉取元数据
    ...
    private long pollUnattachedAsVoter(UnattachedState state, long currentTimeMs) {
        ...
        // 选举超时，变为参与者再进行选举，不断反复直到选举成功
        else if (state.hasElectionTimeoutExpired(currentTimeMs)) {
            transitionToCandidate(currentTimeMs);
            return 0L;
        } else {
            // 更新剩余选举时间，对于非参与者来说，这个是开始选举的倒计时，这个超时时间是个随机值，不同节点可能不一样
            return state.remainingElectionTimeMs(currentTimeMs);
        }
    }
     ...
     */
    public void poll() {
        // 下面这些步骤的顺序为什么这样的 ?muyi

        // 处理注册和取消注册 listener ，取消和注册不是立即生效的，是在这里处理的，单线程执行不立即生效也没事
        pollListeners();
        ...
        // 根据当前状态进行下一步处理 ， 这块很关键是选举、卸任、角色更替的关键
