...
class BrokerServer(
  ...
  override def startup(): Unit = {
    ...
    try {
      ...
      // broker节点生命周期管理，主动上报自己的状态给controller主节点
      ...
      // metadataPublisher 是元数据发生变化后，去做相应的功能处理，例如创建分区文件

      // 如何保证的元数据变更（例如新建分区），最终数据和处理都可最终保持一致；
      // 1.使用者发起请求，leader进行处理，元数据变更
      // 2.follower的broker拉取元数据，发现有变更
      // 3.metadataPublisher进行处理，每次都从上次没有完成的地方开始执行直到追上，元数据变更
      // 4.当上面三个追平时就最终一致了
