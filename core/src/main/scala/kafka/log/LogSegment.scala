...
class LogSegment private[log] (val log: FileRecords,
  ...
  def shouldRoll(rollParams: RollParams): Boolean = {
    ...
    // 1、消息数据大小超过Segment的最大值
    // 2、消息数据大小>Segment剩余空间大小
    // 3、索引和时间索引文件已经满了
    // 4、计算相对偏移量<0 或者 >int最大值 数据的offset - segment第一条数据偏移量
  ...
  def append(largestOffset: Long,
    ...
    if (records.sizeInBytes > 0) {
      ...
      // 如果largestTimestamp 大于 时间索引文件日志最大时间戳 更新时间索引文件
      ...
      if (bytesSinceLastIndexEntry > indexIntervalBytes) {
        // physicalPosition和相对Offset 用 Int存的 所以分区副本文件大小的总字节数不可以大于Int的最大值
