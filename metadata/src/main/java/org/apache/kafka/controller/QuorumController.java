...
public final class QuorumController implements Controller {
    ...
    interface ControllerWriteOperation<T> {
         ...
         * 一旦我们将记录传递到Raft层，我们将使用放置这些记录的结束偏移量调用此函数。
         * 如果没有要写入的记录，我们将只传递最后一个写入偏移量。
         * 写入后数据偏移地址回调
         */
    ...
    class ControllerWriteEvent<T> implements EventQueue.Event, DeferredEvent {
        ...
        public void run() throws Exception {
            ...
            if (result.records().isEmpty()) {
                // 用来回调的，让上层可以感知到 writeOffset 日志偏移地址
            ...
            // 每次元数据发生变更后，如果存在没有topic分区leader副本的broker，那么进行rebalance
            ...
            appendRaftEvent("handleCommit[baseOffset=" + reader.baseOffset() + "]", () -> {
                try {
                    ...
                    while (reader.hasNext()) {
                        ...
                        // leader节点不用replay(messageAndVersion.message(), Optional.empty(), offset);
                        // 因为leader节点不宜哦那个同步自己的日志
    ...
    public CompletableFuture<BrokerRegistrationReply>
        ...
        return appendWriteEvent("registerBroker", () -> {
            // 集群控制管理 clusterControl 开始注册broker
