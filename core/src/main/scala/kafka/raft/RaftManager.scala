...
class KafkaRaftManager[T](
  ...
  def startup(): Unit = {
    ...
    // 获取遍历参与者({id}@{host}:{port}) 并与参与者建立链接
  ...
  // 创建存储数据的文件 log path / __cluster_metadata-0
