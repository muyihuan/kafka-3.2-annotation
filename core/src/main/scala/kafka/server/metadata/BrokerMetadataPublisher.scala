...
class BrokerMetadataPublisher(conf: KafkaConfig,
  ...
  override def publish(delta: MetadataDelta, newImage: MetadataImage): Unit = {
    ...
    try {
      ...
      if (_firstPublish) {
        ...
        // 这里非常关键，当broker进程启动后，注册publisher时会先执行这里，初始化一些很关键的东西，说实话这里不太好
        initializeManagers()
      ...
      // 因为每次重启broker，broker节点第一次都会拉取全量元数据信息，因为重启后读取元数据的起始offset是0
      // 所以第一处理所有的数据就都准备好了 很机智
  ...
  private def finishInitializingReplicaManager(newImage: MetadataImage): Unit = {
    ...
    // 因为每次重启broker，broker节点第一次都会拉取全量元数据信息，因为broker本地不会存储。然后清理不用的分区副本
