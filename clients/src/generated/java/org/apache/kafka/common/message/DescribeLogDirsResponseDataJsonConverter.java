...
public class DescribeLogDirsResponseDataJsonConverter {
    public static DescribeLogDirsResponseData read(JsonNode _node, short _version) {
        ...
        if (_errorCodeNode == null) {
            if (_version >= 3) {
                throw new RuntimeException("DescribeLogDirsResponseData: unable to locate field 'errorCode', which is mandatory in version " + _version);
            } else {
                _object.errorCode = (short) 0;
            }
        } else {
            _object.errorCode = MessageUtil.jsonNodeToShort(_errorCodeNode, "DescribeLogDirsResponseData");
        }
