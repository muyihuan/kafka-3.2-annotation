...
public class CandidateState implements EpochState {
     ...
     *
     * 1. 一旦开始，它将记录收到的选票。
     * 2. 如果获得多数票，它就可以结束它的生命，并将被一个领导国家所取代；
     * 3. 如果多数票被否决或选举超时，它将过渡到后退阶段；退避阶段完成后，它将结束其左侧状态，并被具有缓冲重试的新候选状态替换。
     */
