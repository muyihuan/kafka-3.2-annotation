...
class KafkaRaftServer(
  ...
  // 基于Kraft分布式能力
  // Kraft主要包含：数据存储(日志文件)、数据同步、事件注册监听(数据事件和集群事件)、集群化(选举、心跳等)
  ...
  /**
   * 为什么broker节点不参与选举也要依赖raftManager？
   * 因为broker 需要注册、心跳、离开等 需要controller集群的信息，因为broker节点是多个，存在分布式问题，所以分布式处理统一使用raftManager。
   * raftManager负责了controller集群分布式相关和kafka集群分布式相关
   */
