...
class Partition(val topicPartition: TopicPartition,
  ...
  def updateFollowerFetchState(followerId: Int,
    ...
    getReplica(followerId) match {
      case Some(followerReplica) =>
        ...
        // follower副本如果之前被移出isr了，那么确认是否追上来了，并重新进入isr
  ...
  private def maybeExpandIsr(followerReplica: Replica): Unit = {
    // 当前没有发生isr变化 并且 该follower副本没有在isr里 并且 follower副本的最大offset大于等于当前leader的高水位offset，那么让该副本重新进入isr
  ...
  private def needsShrinkIsr(): Boolean = {
    // Leader的副本负责检查follower副本是否落后了
    leaderLogIfLocal.exists { _ => getOutOfSyncReplicas(replicaLagTimeMaxMs).nonEmpty }
  }

  // follower是否出局
  // 1.follower的logEndOffset 和 leader的EndOffset 不一致 并且 follower上次同步时间已经超过规定时间
  ...
  def appendRecordsToLeader(records: MemoryRecords, origin: AppendOrigin, requiredAcks: Int,
    ...
    val (info, leaderHWIncremented) = inReadLock(leaderIsrUpdateLock) {
      // 该broker上为leader的分区副本
      leaderLogIfLocal match {
          // leader副本
        case Some(leaderLog) =>
          // 最小同步的副本数和acks为-1或"all"配套使用 minInSyncReplicas 可以修改元数据动态配置
          val minIsr = leaderLog.config.minInSyncReplicas
          // 分区isr成员数量
          val inSyncSize = partitionState.isr.size

          // Avoid writing to leader if there are not enough insync replicas to make it safe
          // 如果ack设置为-1表示最高级别，需要把消息同步给所有副本才算消息成功了，并且如果Isr成员数小于配置的最小同步的副本数那么直接失败
          // acks和minInSyncReplicas配置配套使用
